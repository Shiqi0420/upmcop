...
class MOEAD:
    ...
    def run(self):
        ...
        while (t <= self.maxGen):
            ...
            if divisor > 0 and (t >= int(self.maxGen * 0.3)) and (t <= int(self.maxGen * 0.9)) and (t % divisor == 0):
                ...
                if Snew is None:
                    Snew = self.build_similarity_matrix()
                else:
                    # 更新相似度矩阵
                    Snew = self.build_similarity_matrix()

                    # 调用 adapt_weights
                Sold = self.adapt_weights(Sold, Snew, t, self.maxGen)
    ...
    def build_similarity_matrix(self):
        """构建相似度矩阵 Snew"""
        # 这里需要根据你的算法逻辑实现相似度矩阵的构建
        # 暂时返回一个简单的矩阵作为示例
        N = len(self.population)
        Snew = [[0.0 for _ in range(N)] for _ in range(N)]
        ...
        for i in range(N):
            for j in range(N):
                if i != j:
                    # 计算两个个体适应度的欧氏距离作为相似度
                    dist = math.dist(self.population[i].fitness, self.population[j].fitness)
                    Snew[i][j] = 1.0 / (1.0 + dist)  # 转换为相似度
                else:
                    Snew[i][j] = 1.0
    ...
    def ws_transform(self, w):
        """WS-transformation from the paper (Eq.2). input w is list of positive weights that sum to 1."""
        eps = 1e-6
        w_proc = [max(x, eps) for x in w]
        inv = [1.0 / x for x in w_proc]
        s = sum(inv)
        return [v / s for v in inv]

    def initializeWeightVectorAndNeighbor(self):
        """Generate random weight vectors (random sampling) + WS transform, then build neighborhood B.
        This replaces the original fixed 2D linear weighting to support arbitrary objectiveNumber.
        """
        N = int(self.popSize)
        m = int(self.objectNumber)
        # generate random weight vectors via sampling positive random numbers and normalizing
        self.VT = {}
        for i in range(N):
            vec = [random.random() + 1e-6 for _ in range(m)]
            s = sum(vec)
            vec = [v / s for v in vec]
            # apply WS transform
            vec = self.ws_transform(vec)
            self.VT[i] = vec
    ...
    def reproductionPo(self, i):
        # 直接从整个种群中随机选择两个个体，而不是从邻域B[i]中选择
        k = random.randint(0, len(self.population) - 1)
        l = random.randint(0, len(self.population) - 1)
    ...
    def calculate_similarity(self, Snew, Sold, tol=1e-3):
        """Calculate similarity between two matrices Snew and Sold: fraction of elements with abs diff <= tol."""
        if Sold is None:
            return 0.0
        N = len(Snew)
        count_zero = 0
        ...
        for i in range(N):
            for j in range(N):
                if abs(Snew[i][j] - Sold[i][j]) <= tol:
                    count_zero += 1
        return count_zero / float(total) if total > 0 else 0.0
    ...
    def adapt_weights(self, Sold, Snew, gen, Tmax, period_percent=0.05, start=0.2, end=0.9,
        ...
        if frac < start or frac > end:
            return Sold  # no change
        # check period: only check every period_percent fraction of Tmax
        period_gen = max(1, int(Tmax * period_percent))
        if (gen % period_gen) != 0:
            return Sold
        # compute similarity
        sim = self.calculate_similarity(Snew, Sold) if Sold is not None else 0.0
        # proceed only if similarity >= threshold
                ...
                for m in range(self.M):
                    if (self.SeNBSet[m] != self.SeNBSet[i]):
                        for k in range(self.SeNBSet[m].SMDNumber):
                            if (self.SeNBSet[m].SMDSet[k].channel == self.SeNBSet[i].SMDSet[
                                j].channel):  # U_m_j and U_l_i have the same channel
    ...
    def getChannelGain(self, U_i_j,
                       S_i):  # channel gain= D^(-pl), where D is the distance between U_m_j and S_m, pl=4 is the path loss factor
    ...
    def readMECNetwork(self):
        file_SMD_task_cpu = open(
            self.getCurrentPath() + '\\' + self.taskNumberRange + '\SMD_Task_CPU_Cycles_Number.txt', 'r')
        file_SMD_task_data = open(self.getCurrentPath() + '\\' + self.taskNumberRange + '\SMD_Task_Data_Size.txt', 'r')
        file_SMD_output_task_data = open(
            self.getCurrentPath() + '\\' + self.taskNumberRange + '\SMD_Task_Output_Data_Size.txt', 'r')
