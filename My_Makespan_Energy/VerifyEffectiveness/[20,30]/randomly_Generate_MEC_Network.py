...
def randomlyGenerateMECNetwork(MEC_radius, SeNB_radius, SeNB_number, SMD_range, channelNumber, taskNumber_range):
    ...
    for i, crd in enumerate(SeNBCoordinateSet):
        turtle.pencolor('blue')
        x = crd[0]
        y = crd[1]
        ...
        # 使用对应的SeNB半径
        current_SeNB_radius = SeNB_radius[i]
        turtle.penup()
        turtle.goto(x, y-current_SeNB_radius)
        turtle.pendown()
        turtle.circle(current_SeNB_radius)
        ...
        for j in range(SMD_N):
            ...
            for i in range(taskNumber):
                ...
                TaskDataSize_WF.write(str(random.uniform(500, 3000))+'\n')
                outputTaskDataSize_WF.write(str(random.uniform(50, 300))+'\n')
...
Cloud_radius = 160  # The radius of cloud center
SeNB_radius = [100, 90, 80]  # The radius of SeNBs (different for each SeNB, ensuring overlap)
SeNB_number = 3
SMD_range = [3,9]
channelNumber = 10
taskNumber_range = [20, 30]
SeNBCoordinateSet = randomlyGenerateMECNetwork(Cloud_radius, SeNB_radius, SeNB_number, SMD_range, channelNumber, taskNumber_range)
