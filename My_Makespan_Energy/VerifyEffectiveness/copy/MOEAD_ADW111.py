...
class MOEAD:
    def __init__(self, popSize, maxGen, T, taskNumberRange, pc=0.8, pm_SMD=0.03, pm_bit=0.01):
        ...
        # 添加遗传算法参数
        self.pc = pc  # 交叉概率
        self.pm_SMD = pm_SMD  # SMD变异概率
        self.pm_bit = pm_bit  # 基因位变异概率
    ...
    def initializePopulation(self):
        ...
        # 按适应度排序（这里假设是双目标最小化，使用第一个目标值排序）
        combined_pop_sorted = sorted(combined_pop, key=lambda x: x.fitness[0])

        # 选择前 popSize 个最优个体作为初始种群
        self.population = combined_pop_sorted[:self.popSize]
