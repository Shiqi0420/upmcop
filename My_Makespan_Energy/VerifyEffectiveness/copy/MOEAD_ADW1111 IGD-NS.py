...
class MOEAD:
    def __init__(self, popSize, maxGen, T, taskNumberRange):
        ...
        # 初始化参考帕累托前沿 (可用真实PF或近似PF)
        self.reference_PF = []  # 需要在实验中设置或动态更新
        ...
        self.MEC_nodes = [
            {"id": 4, "type": "edge", "compute_capacity": 5, "radius": 50, "power_consumption": 0.8},
            {"id": 5, "type": "edge", "compute_capacity": 4, "radius": 45, "power_consumption": 0.7},
            {"id": 6, "type": "edge", "compute_capacity": 3, "radius": 40, "power_consumption": 0.6},
            {"id": 7, "type": "cloud", "compute_capacity": 50, "radius": 100, "power_consumption": 0}  # 10-16倍
        ]
    ...
    def updateReferencePF(self, population):
        """
        动态更新参考帕累托前沿 (reference_PF)
        从当前种群提取非支配解作为参考集合，逐步逼近真实PF
        """
        # 提取当前种群所有解
        solutions = [ind.fitness for ind in population]

        # 简单非支配排序，筛选一层Pareto前沿
        pareto_front = []
        for sol in solutions:
            dominated = False
            for other in solutions:
                if (other[0] <= sol[0] and other[1] <= sol[1]) and (other != sol):
                    dominated = True
                    break
            if not dominated:
        ...
        # 更新参考PF (合并去重)
        combined = self.reference_PF + pareto_front
        self.reference_PF = [list(x) for x in set(tuple(x) for x in combined)]

        # 保持参考PF按目标1 (时延) 排序
        self.reference_PF.sort(key=lambda x: x[0])
    ...
    def run(self):
        ...
        while (t <= self.maxGen):
            ...
            # 每一代结束后更新参考帕累托前沿
            self.updateReferencePF(self.population)
    ...
    def generateOppositeIndividual(self, ind):
        """
        根据个体 ind 生成对立个体
        对立规则：x' = n + 1 - x，其中 n 是最大执行位置（这里是7）
        """
        opposite_ind = Individual()
        for smd in ind.chromosome:
            opp_smd = copy.deepcopy(smd)
            for j in range(len(opp_smd.workflow.position)):
                # 对立规则：x' = 7 - x
                original_pos = opp_smd.workflow.position[j]
                opposite_pos = 7 - original_pos
                # 确保位置在有效范围内 [1, 7]
                if opposite_pos < 1:
                    opposite_pos = 1
                elif opposite_pos > 7:
                    opposite_pos = 7
                opp_smd.workflow.position[j] = opposite_pos
            opposite_ind.chromosome.append(opp_smd)
        self.calculateFitness(opposite_ind)
        return opposite_ind
    ...
    def initializePopulationWithOBL(self):
        """
        使用OBL机制初始化种群
        """
        P = []  # 原始种群
        ...
        # 生成一半的随机种群
        half_pop_size = int(self.popSize / 2)
        for i in range(half_pop_size):
            ind = self.generateRandomIndividual()
            P.append(ind)

        # 生成对立种群
        for ind in P:
            opp_ind = self.generateOppositeIndividual(ind)
            P_prime.append(opp_ind)

        # 合并两个种群
        combined_pop = P + P_prime

        # 按适应度排序（使用第一个目标值）
        combined_pop_sorted = sorted(combined_pop, key=lambda x: x.fitness[0])

        # 选择前 half_pop_size 个最优个体
        selected_pop = combined_pop_sorted[:half_pop_size]
    ...
    def initializePopulation(self):
        # 使用OBL机制初始化种群
        self.population = self.initializePopulationWithOBL()

        # 设置空的非支配解集和参考点
        self.A = []  # archive set
        self.R = self.initializeUniformReferencePoints()  # 均匀分布的参考点

    def initializeUniformReferencePoints(self):
        """
        初始化均匀分布的参考点
        """
        reference_points = []
        for i in range(self.popSize):
            w1 = i / (self.popSize - 1) if self.popSize > 1 else 0.5
            w2 = 1 - w1
            reference_points.append([w1, w2])
    ...
    def calculateFitness(self, ind):
        ...
        for gene in ind.chromosome:
            ...
            delay_list.append(smd.workflow.schedule.T_total)
            energy_list.append(smd.workflow.schedule.E_total)

        delay = np.average(delay_list)
        energy = np.average(energy_list)
        ind.fitness = [delay, energy]

        # Step 2: IGD
        if hasattr(self, "reference_PF"):
            igd_value = self.calculateIGD([delay, energy], self.reference_PF)
        else:
            igd_value = 0.0

        # Step 3: NS
        ns_value = self.calculateNS([delay, energy])

        # Step 4: IGD-NS 综合
        alpha = 0.5
        ind.igd_ns = igd_value + alpha * ns_value

    def calculateIGD(self, solution, reference_PF):
        """ 计算解到参考帕累托前沿的最小距离 """
        if not reference_PF:  # 如果参考PF为空
            return 0.0  # 或者 return float("inf")，根据你的需求

        distances = [np.linalg.norm(np.array(solution) - np.array(ref)) for ref in reference_PF]
        return np.min(distances) if distances else 0.0


    def calculateNS(self, solution):
        """ 计算解的拥挤度 (简单基于邻居距离) """
        if not hasattr(self, "population") or len(self.population) < 2:
            return 0.0
        distances = []
        for ind in self.population:
            if ind.fitness != solution:
                distances.append(np.linalg.norm(np.array(solution) - np.array(ind.fitness)))
        return np.min(distances) if distances else 0.0
