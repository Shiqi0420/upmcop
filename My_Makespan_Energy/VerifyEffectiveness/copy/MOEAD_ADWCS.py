...
class MOEAD:
    def __init__(self, popSize, maxGen, T, taskNumberRange):
        ...
        self.npa = 50  # WAPD 更新间隔代数
        #---------------------------------------Problem Notation----------------------------------------
        # 修改 ： 定义3个边缘节点和1个云中心计算能力
        self.MEC_nodes = [
    ...
    def run(self):
        ...
        while (t <= self.maxGen):
            ...
            # 每隔 npa 代更新一次权重向量
            if t % self.npa == 0:  # npa 是超参数，例如设为 10
                # 获取当前非支配解集作为种群链
                non_dominated_set = self.get_non_dominated_set()
                if non_dominated_set:
                    self.update_weight_vectors_by_population_distribution(non_dominated_set)
    ...
    def update_weight_vectors_by_population_distribution(self, population_chain):
        """
        根据种群分布动态更新权重向量（WAPD策略）
        :param population_chain: 当前种群的非支配解链（按目标空间排序）
        :return: 新的权重向量列表
        """
        N = len(population_chain)
        if N < 2:
            return self.VT  # 无需更新
        ...
        L = 0.0
        for i in range(N - 1):
            d = self.getDistance(population_chain[i].fitness, population_chain[i + 1].fitness)
            L += d
        ...
        for i in range(N):
            ...
            while c < N - 1:
                l_segment = 0.0
                for j in range(c):
                    d_j = self.getDistance(population_chain[j].fitness, population_chain[j + 1].fitness)
                    l_segment += d_j
                if l_target <= l_segment:
                    break
                c += 1

            if i == 0:
                new_weight_vectors.append(population_chain[0].fitness)
            elif i == N - 1:
                new_weight_vectors.append(population_chain[-1].fitness)
            else:
                # 线性插值
                x_prev = population_chain[c - 1].fitness
                x_curr = population_chain[c].fitness
                d_prev_curr = self.getDistance(x_prev, x_curr)
                gamma = (l_target - l_segment + d_prev_curr) / d_prev_curr
                new_vec = [x_prev[0] + gamma * (x_curr[0] - x_prev[0]),
                           x_prev[1] + gamma * (x_curr[1] - x_prev[1])]
                new_weight_vectors.append(new_vec)

        # 更新 VT 字典
        for i, wv in enumerate(new_weight_vectors):
            self.VT[i] = wv

        return self.VT

    def get_non_dominated_set(self):
        """
        获取当前种群中的非支配解（用于构建种群链）
        """
        non_dominated = []
        for ind in self.population:
            is_dominated = False
            for other in self.population:
                if self.isDominated(other.fitness, ind.fitness):
                    is_dominated = True
                    break
            if not is_dominated:
                non_dominated.append(ind)
        # 按第一个目标值排序
        non_dominated.sort(key=lambda x: x.fitness[0])
            ...
            for senb in self.SeNBSet:
                for smd in senb.SMDSet:
                    ...
                    for j in range(temp_smd.workflow.taskNumber):
                        ...
                        # connected_node = self.getConnectedMECNodeImproved(temp_smd.coordinate, senb.coordinate, task,temp_smd)
                        # 计算本地执行时间（三个核心的平均值）
    ...
    # 基于一种启发式选择
    # def getConnectedMECNodeImproved(self, smd_coordinate, senb_coordinate, task, temp_smd, alpha=0.01, beta=0.99,
    #                                  gamma=0.1):
    #     distance_to_senb = self.getDistance(smd_coordinate, senb_coordinate)
    #     best_node = None
    ...
    #     for node in self.MEC_nodes:
    #         distance_to_node = distance_to_senb
    #         if distance_to_node > node["radius"]:
    #             continue  # 该节点不可达
    #         # 本地执行时延和能耗
    #         T_local = (task.c_i_j_k / temp_smd.coreCC[1] +
    #                    task.c_i_j_k / temp_smd.coreCC[2] +
    #                    task.c_i_j_k / temp_smd.coreCC[3]) / 3
    #         E_local = min(temp_smd.pcc_i_j.values()) * T_local
    #
    #         if node["type"] == "edge":
    #             # 计算边缘节点的时延和能耗
    #             T_edge = (task.d_i_j_k + task.o_i_j_k) / temp_smd.R_i_j + task.c_i_j_k / node["compute_capacity"]
    #             E_edge = temp_smd.pws_i_j * (task.d_i_j_k / temp_smd.R_i_j) \
    #                      + temp_smd.pwr_i_j * (task.o_i_j_k / temp_smd.R_i_j) \
    #                      + node["power_consumption"] * (task.c_i_j_k / node["compute_capacity"])
    #             delay, energy = T_edge, E_edge
    #
    #         elif node["type"] == "cloud":
    #             # 计算云中心的时延和能耗
    #             T_cloud = (task.d_i_j_k + task.o_i_j_k) / temp_smd.R_i_j \
    #                       + task.d_i_j_k / self.cloud_bandwidth \
    #                       + task.c_i_j_k / node["compute_capacity"]
    #             E_cloud = temp_smd.pws_i_j * (task.d_i_j_k / temp_smd.R_i_j)
    #             delay, energy = T_cloud, E_cloud
    #         else:
    #             continue
    #         # 综合评分，包含时延、能耗以及负载均衡
    #         score = alpha * delay + beta * energy + gamma * (1 / node["compute_capacity"])  # 更加注重负载
    #         if score < best_score:
    #             best_score = score
    #             best_node = node
    #     return best_node
