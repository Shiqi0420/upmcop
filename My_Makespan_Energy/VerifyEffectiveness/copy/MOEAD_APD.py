...
class MOEAD:
    ...
    def run(self):
        ...
        for t in range(1, self.maxGen + 1):
            print('Generation--', t)
            ...
            for i in range(self.popSize):
                ...
                self.newExternalPopulation(y_)

            if t <= self.maxGen * 0.9:  # 前90%的代数更新外部种群
                self.updateExternalPopulation(t)

            divisor = int(self.maxGen * 0.05)
            if divisor > 0 and (t >= int(self.maxGen * 0.2)) and (t <= int(self.maxGen * 0.9)) and (t % divisor == 0):
                self.updateWeightVectors(t)


        return [copy.deepcopy(ind.fitness) for ind in self.population]

    """
        **********************************************run**********************************************
    """

    def calculateAPD(self, ind, weight_vector, current_gen):
        # 角度惩罚距离 (Angle Penalized Distance)
        norm_f = np.array(ind.fitness) - np.array(self.Z)
        norm_w = np.array(weight_vector) / np.linalg.norm(weight_vector)

        d1 = abs(np.dot(norm_f, norm_w)) / np.linalg.norm(norm_w)
        d2 = np.linalg.norm(norm_f - d1 * norm_w)
        theta = math.acos(np.dot(norm_f, norm_w) / (np.linalg.norm(norm_f) * np.linalg.norm(norm_w) + 1e-9))
        apd = d1 + (current_gen / self.maxGen) * d2 * theta
        return apd

    def newExternalPopulation(self, offspring):
        self.EP.append(copy.deepcopy(offspring))

    def updateExternalPopulation(self, t):
        # 计算每个解的 APD 值
        apd_values = []
        for j, ep in enumerate(self.EP):
            w = random.choice(list(self.VT.values()))
            apd = self.calculateAPD(ep, w, t)
            apd_values.append((j, apd))

        # 按 APD 值排序
        apd_values.sort(key=lambda x: x[1], reverse=True)  # 降序排序

        # 维护 EP 的大小不超过 2N
        while len(self.EP) > 2 * self.popSize:
            # 移除 APD 值最大的解
            idx_to_remove = apd_values[0][0]
            self.EP.pop(idx_to_remove)
            # 更新 apd_values
            apd_values = [(i, apd) for i, (idx, apd) in enumerate(apd_values) if idx != idx_to_remove]

    def updateWeightVectors(self, current_gen):
        """
        算法4：更新权重向量
        根据外部种群EP调整权重向量、种群和邻域
        返回: 更新后的种群P, 权重向量λ, 邻域B
        """
        # 记录需要更新的索引
        updated_indices = []

        # 从 EP（外部种群）中挑选优秀解来生成新的权重向量
        for ep in self.EP:
            # 找到当前种群中与当前外部解 ep 最接近的 m 个个体（m 为目标数）
            distances = [(i, self.getDistance(ep.fitness, ind.fitness))
                         for i, ind in enumerate(self.population)]
            distances = sorted(distances, key=lambda x: x[1])
            nearest_indices = [i for i, _ in distances[:self.objectNumber]]
            nearest_set = [self.population[i] for i in nearest_indices]

            # 判断是否需要进行替换操作
            apd_values = [self.calculateAPD(ind, self.VT[idx], current_gen)
                          for idx, ind in zip(nearest_indices, nearest_set)]
            worst_idx = np.argmax(apd_values)

            if self.calculateAPD(ep, random.choice(list(self.VT.values())), current_gen) < apd_values[worst_idx]:
                # 替换最差的个体及其对应的权重向量
                worst_population_idx = nearest_indices[worst_idx]
    ...
    def initializePopulation(self):
        ...
        #                         pos_range = [1, 2, 3, connected_node["id"]]
        #                     else:  # cloud
        #                         pos_range = [1, 2, 3, 4, 5, 6, 7]
        #                 else:
