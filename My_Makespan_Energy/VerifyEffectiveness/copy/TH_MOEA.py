...
class THMOEA:
    def __init__(self, popSize, maxGen, T, taskNumberRange, theta=0.4):
        self.popSize = popSize
        self.maxGen = maxGen
        self.T = T  # 邻居数量
        self.taskNumberRange = taskNumberRange
        self.theta = theta  # 两阶段切换阈值
        self.VT = {}  # 权重向量集合
        self.B = {}  # 权向量的邻居
        self.population = []
        self.Z = []  # 参考点
    ...
    def NCSO_evolution(self):
        """第一阶段：使用NCSO进行进化"""
        # 分割种群为winner和loser
        W, L = self.split_population_by_dominance()

        # 基于角度匹配winner和loser
        matched_pairs = self.match_by_angle(W, L)

        # 更新loser粒子
        for loser, winner in matched_pairs:
            self.update_loser_from_winner(loser, winner)
            # 对winner粒子进行多项式变异
            self.polynomial_mutation(winner)

        # 重新计算适应度
        for ind in self.population:
            self.calculateFitness(ind)

    def DIA_evolution(self):
        """第二阶段：使用DIA进行进化"""
        offspring = []
        for i, ind in enumerate(self.population):
            # 计算垂直距离
            vd = self.calculate_vertical_distance(ind, i)
            # 计算克隆数量
            clone_num = self.calculate_clone_number(vd)
            # 克隆个体
            for _ in range(clone_num):
                clone_ind = copy.deepcopy(ind)
                # 对克隆个体进行变异
                self.mutantOperator(clone_ind)
                self.calculateFitness(clone_ind)
                offspring.append(clone_ind)

        # 更新外部档案EP
        for ind in offspring:
            self.update_EP_FromElement(self.EP, ind)

        # 从 offspring 和当前种群中选择下一代
        self.environmental_selection(offspring)

    def split_population_by_dominance(self):
        """基于Pareto支配关系分割种群"""
        W = []  # winner粒子
        L = []  # loser粒子

        # 简单实现：基于非支配排序选择前一半作为winner
        self.fast_non_dominated_sort(self.population)
        sorted_pop = []
        for front in self.F_rank[1:]:
            sorted_pop.extend(front)

        half_size = len(sorted_pop) // 2
        W = sorted_pop[:half_size]
        L = sorted_pop[half_size:]

        return W, L

    def match_by_angle(self, W, L):
        """基于角度信息匹配winner和loser粒子"""
        matched_pairs = []
        ...
        for loser in L:
            ...
            for winner in W:
                angle = self.calculate_angle(loser, winner)
                if angle < min_angle:
                    min_angle = angle
                    best_winner = winner

            if best_winner is not None:
                matched_pairs.append((loser, best_winner))

        return matched_pairs

    def calculate_angle(self, ind1, ind2):
        """计算两个个体之间的角度"""
        f1 = np.array(ind1.fitness)
        f2 = np.array(ind2.fitness)
    ...
    def update_loser_from_winner(self, loser, winner):
        """loser粒子向winner粒子学习"""
        # 简化的学习策略：直接替换部分基因
        for i in range(len(loser.chromosome)):
            if random.random() < 0.5:  # 50%的概率学习
                loser.chromosome[i] = copy.deepcopy(winner.chromosome[i])

    def polynomial_mutation(self, ind):
        """多项式变异"""
        self.mutantOperator(ind)

    def calculate_vertical_distance(self, ind, index):
        """计算垂直距离"""
        # 简化的垂直距离计算
        f = np.array(ind.fitness)
        z = np.array(self.Z)

        # 计算到参考点的距离
        return np.linalg.norm(f - z)

    def calculate_clone_number(self, vd):
        """基于垂直距离计算克隆数量"""
        # 简化的克隆策略：垂直距离越小，克隆数量越多
        base_clone = 1
        if vd < 0.1:  # 垂直距离很小
            return base_clone + 2
        elif vd < 0.5:  # 垂直距离中等
    ...
    def environmental_selection(self, offspring):
        ...
        self.fast_non_dominated_sort(combined_pop)

        # 选择前popSize个个体
        new_pop = []
        for front in self.F_rank[1:]:
            if len(new_pop) + len(front) <= self.popSize:
                new_pop.extend(front)
            else:
                # 需要从当前前沿中选择部分个体
                remaining = self.popSize - len(new_pop)
                # 简单策略：随机选择
                selected = random.sample(front, remaining)
                new_pop.extend(selected)
                break

        self.population = new_pop[:self.popSize]
