...
class NSGA2:
    def __init__(self, popSize, maxGen, taskNumberRange):
    ...
    def run(self):
        ...
        while gen <= self.maxGen:
            ...
            next_pop = []
            front_idx = 1
            while len(next_pop) + len(self.F_rank[front_idx]) <= self.popSize:
                next_pop.extend(self.F_rank[front_idx])
                front_idx += 1

            # 如果未满，按拥挤度排序补足
            if len(next_pop) < self.popSize:
                remaining = self.popSize - len(next_pop)
                self.F_rank[front_idx].sort(key=lambda x: x.distance, reverse=True)
                next_pop.extend(self.F_rank[front_idx][:remaining])
        ...
        # 最终提取 Pareto 前沿
        self.fast_non_dominated_sort(self.population)
        self.EP = self.F_rank[1]  # 第一层为非支配解
    ...
    def tournament_selection(self):
        ...
        # 按适应度排序
        selected_indices.sort(key=lambda idx: (self.population[idx].rank, -self.population[idx].distance))
        return selected_indices[0]  # 返回最佳个体的索引


    def generate_offspring(self):
        offspring = []
        for _ in range(self.popSize):
            k = self.tournament_selection()
            l = self.tournament_selection()
            ind_k = Individual()
            ind_l = Individual()
            ...
            for gene in self.population[l].chromosome:
                smd_l = SMD()
                smd_l.workflow.position = copy.deepcopy(gene.workflow.position)
                smd_l.workflow.sequence = copy.deepcopy(gene.workflow.sequence)
                ind_l.chromosome.append(smd_l)

            self.crossoverOperator(ind_k, ind_l)
            self.mutantOperator(ind_k)
            self.calculateFitness(ind_k)
            offspring.append(ind_k)
        return offspring
